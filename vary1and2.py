...
"""
Breast -> vary 1st -> vary 2nd -> end site
"""
...
thispath = ('Breast|Lung/pleura|Liver|Deceased')
print "Interested in ", thispath, " path"
thispath_temp = thispath.split('|')
thispath_start = "|".join([thispath_temp[0],thispath_temp[1], thispath_temp[2]])
thispath_fs = "|".join([thispath_temp[0],thispath_temp[1]])
thispath_fe = "|".join([thispath_temp[3]]) 

for path in all_paths:
    if str(path[0]).startswith(thispath_start):
...
for site1 in all_sites:
    for site2 in all_sites:
        thatpath = "|".join([thispath_temp[0],site1,site2,thispath_fe])
        thatpath_firstmet = "|".join([thispath_temp[0],site1,site2])
        for path in all_paths:
