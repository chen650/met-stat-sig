...
for thispath in all_paths: #choose first path from paths in excel sheet
    ...
    for site in top_sites: #iterating thru all met sites to choose comparison path
        thatpath = "|".join([origin,firstmet,site,thirdmet]) #choose comparison path
        thatpath_firstmet = "|".join([origin,firstmet,site])      
...
with open('/Users/Confetti/Documents/Research/listvary2/Vary2_OR_Paths_Bladder.csv', 'wb') as f: #CHANGE
    writer = csv.writer(f, delimiter=',')
    for key in or_arr:
        writer.writerow(key)

with open('/Users/Confetti/Documents/Research/listvary2/Vary2_AOR_Paths_Bladder.csv', 'wb') as f: #CHANGE
    writer = csv.writer(f, delimiter=',')
