...
all_pat = pd.read_excel('/Users/Confetti/Documents/pathways.xlsx',sheetname='hcv',header=None) #CHANGE
...
for k,v in all_paths.items(): 
    ...
    if len(path_temp) < 3 or path_temp[2] == 'Left Study':
        del all_paths[k]
...
for k,v in all_paths.items(): #delete patients taken out b/c of 2.5% rule
    ...
    if path_temp[1] not in top_sites or path_temp[2] not in top_sites:
        del all_paths[k];
...
for thispath in all_paths: #choose first path from paths in excel sheet
    ...
    thispath_temp = [thispath_temp[0], \
        thispath_temp[1], thispath_temp[2]] #truncate to origin and 2 mets
    thispath = "|".join([thispath_temp[0], thispath_temp[1], thispath_temp[2]])
    origin = thispath_temp[0]
    thispath_firstmet = "|".join([origin, thispath_temp[1]]) 
    ...
    for path in all_paths: #necessary bc you want to ignore later mets
        ...
        if str(path).startswith(thispath_firstmet):
            thispath_ct1 = thispath_ct1 + all_paths[path]; 
            if str(path).startswith(thispath):
                thispath_ct2 = thispath_ct2 + all_paths[path];
...
with open('/Users/Confetti/Documents/Research/list2step/OR_Paths_HCV.csv', 'wb') as f: #CHANGE
    writer = csv.writer(f, delimiter=',')
    for key in or_arr:
        writer.writerow(key)

with open('/Users/Confetti/Documents/Research/list2step/AOR_Paths_HCV.csv', 'wb') as f: #CHANGE
    writer = csv.writer(f, delimiter=',')
