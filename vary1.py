...
"""
Breast -> vary 1st -> 2nd site -> end site
"""
...
thispath = ('Breast|Bone|Liver|Lung/pleura')
thispath_temp = thispath.split('|')
thispath_start = "|".join([thispath_temp[0],thispath_temp[1], thispath_temp[2]])
thispath_fs = "|".join([thispath_temp[0]])
thispath_fe = "|".join([thispath_temp[2],thispath_temp[3]]) 
...
for site in all_sites:
    thatpath = "|".join([thispath_fs,site,thispath_fe])
    thatpath_firstmet = "|".join([thispath_fs,site,thispath_temp[2]])       
    for path in all_paths:
